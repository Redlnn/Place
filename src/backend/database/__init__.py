...
class Database:
    ...
    async def init(cls, debug: bool = False) -> None:
        ...
        if not DATABASE_URL.startswith('sqlite') and not DATABASE_URL.startswith('mysql+asyncmy'):
            raise ValueError('不支持的数据库类型')
        cls.engine = create_async_engine(DATABASE_URL, echo=debug, future=True, pool_pre_ping=True, pool_recycle=180)
